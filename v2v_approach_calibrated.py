#!/usr/bin/env python3
"""
V2V Approach 5: Fine-Tuned Calibration
...
def analyze_calibration_errors(waypoints_df):
    """Analyze current errors to find optimal calibration factors per zone"""
    print("🔍 Analyzing calibration errors for zone optimization...")
    
    # Load existing analysis if available
    try:
        existing_analysis = pd.read_csv('realistic_speed_waypoint_analysis.csv')
        print(f"✅ Loaded existing analysis: {len(existing_analysis)} waypoints")
        
        # Calculate optimal calibration per waypoint cluster
        errors = existing_analysis['distance_error_m']
        actual_distances = existing_analysis['actual_distance_m']
        simulated_distances = existing_analysis['simulated_distance_m']
        
        # Calculate optimal factor per waypoint
        optimal_factors = actual_distances / simulated_distances
        
        # Group by waypoint ranges and calculate zone-based factors
        zone_factors = {}
        zone_ranges = [
            (0, 50, 'zone_1'),
            (50, 100, 'zone_2'), 
        ...
        for start, end, zone_name in zone_ranges:
            zone_mask = (existing_analysis['waypoint'] >= start) & (existing_analysis['waypoint'] < end)
            if zone_mask.sum() > 0:
                zone_optimal = optimal_factors[zone_mask]
                zone_factor = zone_optimal.mean()
                zone_factors[zone_name] = {
                    'factor': zone_factor,
                    'waypoints': range(start, end),
                    'count': zone_mask.sum(),
                    'std': zone_optimal.std()
                }
                print(f"📊 {zone_name}: factor={zone_factor:.3f}, count={zone_mask.sum()}, std={zone_optimal.std():.3f}")
        
        return zone_factors
        
    except FileNotFoundError:
        print("⚠️ No existing analysis found, using default zone factors")
        # Default zone-based calibration factors
        return {
            'zone_1': {'factor': 0.58, 'waypoints': range(0, 50), 'count': 0, 'std': 0.0},
            'zone_2': {'factor': 0.62, 'waypoints': range(50, 100), 'count': 0, 'std': 0.0},
            'zone_3': {'factor': 0.61, 'waypoints': range(100, 150), 'count': 0, 'std': 0.0},
            'zone_4': {'factor': 0.59, 'waypoints': range(150, 200), 'count': 0, 'std': 0.0}
        }

class V2VFineTunedCalibrationSimulation:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("V2V Approach 5: Fine-Tuned Calibration")
    ...
    def setup_gui(self):
        ...
        title_label = tk.Label(self.root, text="V2V Approach 5: Fine-Tuned Calibration", 
                              font=("Arial", 16, "bold"))
        title_label.pack(pady=10)
        
        info_label = tk.Label(self.root, text="Zone-based calibration factors optimized per waypoint region!", 
        ...
        ttk.Checkbutton(config_frame, text="✅ Apply Fine-Tuned Calibration", 
    ...
    def run_simulation(self):
        """Main simulation with FINE-TUNED CALIBRATION"""
        try:
            self.log_message("="*70)
            self.log_message("V2V APPROACH 5: FINE-TUNED CALIBRATION")
            self.log_message("="*70)
            self.log_message(f"🚗 Speed Mode: {'REALISTIC (from GPS data)' if self.use_realistic_speed.get() else 'CONSTANT (15 m/s)'}")
            self.log_message(f"📊 Calibration: {'FINE-TUNED ZONE-BASED' if self.calibration_enabled.get() else 'DISABLED'}")
            ...
            # FINE-TUNED: Analyze calibration errors for zone optimization
            self.log_message(f"\n🔍 Analyzing calibration errors for zone optimization...")
            calibration_zones = analyze_calibration_errors(waypoints_df)
            
            self.log_message(f"\n📊 Fine-Tuned Calibration Zones:")
            for zone_name, zone_info in calibration_zones.items():
                self.log_message(f"   {zone_name}: factor={zone_info['factor']:.3f}, "
                               f"waypoints={zone_info['waypoints'].start}-{zone_info['waypoints'].stop-1}, "
                               f"count={zone_info['count']}")
            ...
            # Create route files
            with open('fine_tuned_source_route.rou.xml', 'w') as f:
                f.write('<?xml version="1.0" encoding="UTF-8"?>\n')
                f.write('<routes>\n')
                f.write('  <route id="fine_tuned_source_route" edges="' + ' '.join(source_route) + '"/>\n')
                f.write('  <route id="fine_tuned_dest_route" edges="' + ' '.join(dest_route) + '"/>\n')
                f.write('  <vType id="fine_tuned_vType" accel="2.0" decel="4.5" sigma="0.5" length="4.5" maxSpeed="50" guiShape="passenger"/>\n')
                f.write('</routes>\n')
            
            # Create SUMO config
            with open('fine_tuned.sumocfg', 'w') as f:
                f.write('<?xml version="1.0" encoding="UTF-8"?>\n')
                f.write('<configuration>\n')
                f.write('  <input>\n')
                f.write('    <net-file value="osm.net.xml.gz"/>\n')
                f.write('    <route-files value="fine_tuned_source_route.rou.xml"/>\n')
                f.write('  </input>\n')
                f.write('  <time>\n')
            ...
            self.log_message(f"\n🚀 Starting SUMO-GUI with fine-tuned calibration...")
            
            traci.start(sumo_cmd)
            time.sleep(2)
            
            # Add vehicles
            traci.vehicle.add("fine_tuned_v2v_source", "fine_tuned_source_route", typeID="fine_tuned_vType")
            traci.vehicle.add("fine_tuned_v2v_dest", "fine_tuned_dest_route", typeID="fine_tuned_vType")
            
            # Set vehicle properties
            traci.vehicle.setColor("fine_tuned_v2v_source", (0, 0, 255, 255))  # Blue
            traci.vehicle.setColor("fine_tuned_v2v_dest", (255, 0, 0, 255))    # Red
            
            self.log_message(f"✅ Vehicles added with fine-tuned calibration")
            
            # Prepare waypoint coordinates for fine-tuned analysis
            waypoint_coords = []
            for idx, row in waypoints_df.iterrows():
            ...
            # Simulation loop with FINE-TUNED CALIBRATION
            self.log_message(f"\n🎯 Starting simulation with fine-tuned calibration...")
            ...
            while step < SIMULATION_STEPS and self.simulation_running:
                ...
                # Source vehicle
                if "fine_tuned_v2v_source" in vehicles:
                    if not source_active:
                        self.log_message(f"✅ Step {step}: Source vehicle active")
                        source_active = True
                    
                    src_pos = traci.vehicle.getPosition("fine_tuned_v2v_source")
                    src_speed = traci.vehicle.getSpeed("fine_tuned_v2v_source")
                    ...
                    if self.use_realistic_speed.get() and current_waypoint < len(waypoint_coords):
                        ...
                        traci.vehicle.setSpeed("fine_tuned_v2v_source", target_speed_ms)
                ...
                # Destination vehicle
                if "fine_tuned_v2v_dest" in vehicles:
                    if not dest_active:
                        self.log_message(f"✅ Step {step}: Destination vehicle active")
                        dest_active = True
                    
                    dst_pos = traci.vehicle.getPosition("fine_tuned_v2v_dest")
                    dst_speed = traci.vehicle.getSpeed("fine_tuned_v2v_dest")
                    
                    # SET REALISTIC SPEED from dataset
                    if self.use_realistic_speed.get() and current_waypoint < len(waypoint_coords):
                        target_speed_ms = kmh_to_ms(waypoint_coords[current_waypoint]['dest_speed_kmh'])
                        traci.vehicle.setSpeed("fine_tuned_v2v_dest", target_speed_ms)
                ...
                if source_active and dest_active:
                    ...
                    # FINE-TUNED: Apply zone-based calibration
                    if self.calibration_enabled.get():
                        # Determine which zone the current waypoint belongs to
                        current_calibration = BASE_CALIBRATION_FACTOR  # Default
                    ...
                    if current_waypoint != prev_waypoint and current_waypoint < len(waypoint_coords):
                        ...
                        waypoint_analysis.append({
                            ...
                            'approach': 'fine_tuned_calibration',
                            'calibration_zone': calibration_zone,
                            'calibration_factor_used': current_calibration
                        })
                    ...
                    if step - last_log_step >= 200:
                        ...
                        self.log_message(f"📊 Step {step}: Distance={avg_calib:.2f}m, "
                                        ...
                                        f"Zone={calibration_zone}, Factor={current_calibration:.3f}")
                        last_log_step = step
            ...
            if distances_raw:
                ...
                self.log_message(f"   Fine-Tuned Calibrated: avg={sum(distances_calibrated)/len(distances_calibrated):.2f}m")
            ...
            self.log_message(f"📊 Calibration: FINE-TUNED ZONE-BASED")
            ...
            if waypoint_analysis:
                ...
                csv_file = os.path.join(original_dir, 'approach_5_fine_tuned_calib_analysis.csv')
                ...
                zone_analysis = {}
                for zone_name, zone_info in calibration_zones.items():
                    zone_mask = analysis_df['calibration_zone'] == zone_name
                    if zone_mask.sum() > 0:
                        zone_accuracies = distance_accuracies[zone_mask]
                        zone_analysis[zone_name] = {
                            'count': int(zone_mask.sum()),
                            'mean_accuracy': float(zone_accuracies.mean()),
                            'median_accuracy': float(zone_accuracies.median()),
                            'calibration_factor': zone_info['factor']
                        }
                ...
                overall_metrics = {
                    'simulation_settings': {
                        ...
                        'calibration_enabled': self.calibration_enabled.get(),
                        'base_calibration_factor': BASE_CALIBRATION_FACTOR,
                        'realistic_speed_enabled': self.use_realistic_speed.get(),
                ...
                json_file = os.path.join(original_dir, 'approach_5_fine_tuned_calib_summary.json')
                ...
                self.log_message("\n" + "="*70)
                self.log_message("APPROACH 5: FINE-TUNED CALIBRATION - ACCURACY REPORT")
                self.log_message("="*70)
                ...
                for zone_name, zone_info in zone_analysis.items():
                    self.log_message(f"   {zone_name}: {zone_info['count']} waypoints, "
                                   f"accuracy={zone_info['mean_accuracy']:.2f}%, "
                                   f"factor={zone_info['calibration_factor']:.3f}")
                ...
                self.log_message(f"   📄 CSV: approach_5_fine_tuned_calib_analysis.csv")
                self.log_message(f"   📄 JSON: approach_5_fine_tuned_calib_summary.json")
            ...
            self.log_message("\n🔍 Check SUMO-GUI:")
            self.log_message("   - Blue vehicle should follow fine-tuned trajectory")
            self.log_message("   - Red vehicle should follow fine-tuned trajectory")
            self.log_message("   - Both vehicles should stay on roads")
            self.log_message("   - Processed waypoints with fine-tuned calibration")
...
if __name__ == "__main__":
    app = V2VFineTunedCalibrationSimulation()
    app.run()
