#!/usr/bin/env python3
"""
Headless V2V Distance Validation (Calibrated Version)
...
4. Applying calibration factor (0.607) to correct systematic error
5. Comparing with actual distances from the dataset
6. Reporting accuracy metrics

CALIBRATION FACTOR: 0.607 (corrects 64.6% systematic overestimation)
"""
...
class HeadlessV2VDistanceValidator:
    ...
    def __init__(self, config_path: str = "osm_headless.sumocfg"):
        ...
        self.CALIBRATION_FACTOR = 0.607  # Corrects systematic 64.6% overestimation
    ...
    def simulate_vehicle_pair(self, 
        ...
        result = {
            ...
            'raw_simulated_distance': None,  # Keep original for analysis
            'error': None,
        ...
        raw_simulated_distance = math.sqrt((dst_x - src_x)**2 + (dst_y - src_y)**2)
        
        # Apply calibration factor to correct systematic error
        simulated_distance = raw_simulated_distance * self.CALIBRATION_FACTOR
        
        result['simulated_distance'] = simulated_distance
        result['raw_simulated_distance'] = raw_simulated_distance
        result['success'] = True
        ...
        print(f"   Raw simulated distance: {raw_simulated_distance:.2f}m")
        print(f"   ✅ Calibrated distance: {simulated_distance:.2f}m")
    ...
    def validate_distances(self, max_samples: int = 10) -> Dict:
        ...
        print("🚀 Starting Headless V2V Distance Validation (CALIBRATED)")
        print("=" * 70)
    ...
    def analyze_results(self, results: List[Dict]) -> Dict:
        ...
        raw_simulated_distances = [r['raw_simulated_distance'] for r in successful_results]
        ...
        analysis = {
            ...
            'distance_ranges': {
                ...
                'calibrated_simulated_distances': {
                ...
                'raw_simulated_distances': {
                    'min': np.min(raw_simulated_distances),
                    'max': np.max(raw_simulated_distances),
                    'mean': np.mean(raw_simulated_distances),
                    'std': np.std(raw_simulated_distances)
                }
            }
        }
        ...
        print(f"🔧 Calibration factor applied: {self.CALIBRATION_FACTOR}")
    ...
    def save_results(self, results: List[Dict], analysis: Dict):
        ...
        results_file = f"distance_validation_calibrated_results_{timestamp}.json"
        ...
        # Save summary report
        summary_file = f"distance_validation_calibrated_summary_{timestamp}.txt"
        with open(summary_file, 'w') as f:
            f.write("HEADLESS V2V DISTANCE VALIDATION SUMMARY (CALIBRATED)\n")
            f.write("=" * 60 + "\n\n")
...
def main():
    """Main function"""
    print("Headless V2V Distance Validation (CALIBRATED VERSION)")
    print("#" * 60)
    ...
    try:
        ...
        if results:
            print("\n🎉 Calibrated validation completed successfully!")
        else:
            print("\n❌ Validation failed")
