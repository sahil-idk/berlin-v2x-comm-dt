#!/usr/bin/env python3
"""
Convert CSV data to embedded JavaScript for HTML visualization
...
def convert_csv_to_embedded_html():
    """Convert CSV data to embedded JavaScript in HTML file"""
    
    print("🔄 Converting CSV to embedded HTML...")
    
    # Read the closer points CSV
    csv_file = "vehicle_2_4_closer_points.csv"
    ...
    html_file = "closer_v2v_visualization.html"
    ...
    old_csv_loading = '''// Load closer points CSV data
function loadCsvData() {
  setInfoText('Loading closer Vehicle 2-4 dataset...');
  
  Papa.parse('vehicle_2_4_closer_points.csv', {
    ...
    complete: function(results){
      ...
      setInfoText(`Loaded ${allData.length} closer V2V communication records`);
      ...
      document.getElementById('distanceRange').textContent = '2.6m - 25.0m';
    ...
    new_csv_loading = f'''// Load embedded data (no CSV file needed)
function loadCsvData() {{
  setInfoText('Loading embedded Vehicle 2-4 dataset...');
...
// Embedded Vehicle 2-4 closer points data
...
'''
    ...
    output_file = "closer_v2v_visualization_embedded.html"
