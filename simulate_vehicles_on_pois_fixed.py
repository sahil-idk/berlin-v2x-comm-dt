#!/usr/bin/env python3
"""
Fixed TraCI Vehicle Simulation on Lane-Mapped POIs
Simulates vehicles directly on GPS waypoints using existing SUMO configuration
"""
...
def simulate_vehicles_on_pois_fixed():
    """Simulate vehicles directly on lane-mapped POI points"""
    
    print("=" * 70)
    print("Fixed TraCI Vehicle Simulation on Lane-Mapped POIs")
    print("=" * 70)
    ...
    # Get available vehicle types from the simulation
    try:
        vehicle_types = traci.vehicletype.getIDList()
        print(f"Available vehicle types: {vehicle_types}")
        
        if vehicle_types:
            default_type = vehicle_types[0]  # Use first available type
        else:
            default_type = "DEFAULT_VEHTYPE"  # Fallback
    except:
        default_type = "DEFAULT_VEHTYPE"
    
    print(f"Using vehicle type: {default_type}")
    ...
    try:
        ...
        if routes:
            default_route = routes[0]  # Use first available route
        else:
            default_route = "route0"  # Fallback
    except:
        default_route = "route0"
    
    print(f"Using route: {default_route}")
    
    # Add vehicles with proper error handling
    try:
        traci.vehicle.add("sourceVehicle", default_route, typeID=default_type)
        print("✅ Source vehicle added successfully")
    except Exception as e:
        print(f"❌ Failed to add source vehicle: {e}")
        print("Continuing with POI visualization only...")
        source_vehicle_added = False
    else:
        source_vehicle_added = True
    
    try:
        traci.vehicle.add("destVehicle", default_route, typeID=default_type)
        print("✅ Destination vehicle added successfully")
    except Exception as e:
        print(f"❌ Failed to add destination vehicle: {e}")
        print("Continuing with POI visualization only...")
        dest_vehicle_added = False
    else:
        dest_vehicle_added = True
    
    if source_vehicle_added and dest_vehicle_added:
        # Set vehicle colors
        try:
            ...
            print("✅ Vehicle properties set successfully")
        except Exception as e:
            print(f"⚠️  Warning: Could not set vehicle properties: {e}")
        ...
        while step < max_steps and point_index < len(lane_mapped_points):
            ...
            if step % 30 == 0 and point_index < len(lane_mapped_points):
                ...
                if current_point['vehicle'] == 2 and source_vehicle_added:  # Source vehicle
                    try:
                ...
                elif current_point['vehicle'] == 4 and dest_vehicle_added:  # Destination vehicle
                    try:
            ...
            if step % 10 == 0 and source_vehicle_added and dest_vehicle_added:
                try:
        ...
        print("\n⚠️  Vehicle simulation skipped due to vehicle addition errors")
        print("🔍 Check SUMO-GUI:")
        print("  - Blue POI markers show source waypoints")
        print("  - Red POI markers show destination waypoints")
        print("  - All POIs are mapped to road lanes")
...
if __name__ == "__main__":
    simulate_vehicles_on_pois_fixed()
